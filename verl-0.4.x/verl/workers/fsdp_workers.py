...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        if self.config.actor.get("freeze_embedding", False):
            emb = actor_module.get_input_embeddings()
            if emb is not None:
                emb.weight.requires_grad_(False)
                self.use_orig_params = True  # Must use orig_params when freezing embedding
                if self.rank == 0:
                    print("[actor model] Embedding layer is set to not trainable.")
            head = actor_module.get_output_embeddings()
            if head is not None:
    ...
    def set_actor_lr(self, lr):
        """Manually set the learning rate for the actor optimizer."""
        assert self._is_actor
        if self.actor_optimizer is not None:
            # Update optimizer param groups
            for param_group in self.actor_optimizer.param_groups:
                param_group['lr'] = lr
                # Also update initial_lr if present, though scheduler might use base_lrs
                if 'initial_lr' in param_group:
                    param_group['initial_lr'] = lr
            
            # Update scheduler base_lrs to ensure step() doesn't revert it
            ...
            if self.rank == 0:
                print(f"Manually set actor learning rate to {lr}")
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            with adapter_ctx:
                output, entropys, topk_probs, topk_ids, topk_logits = self.actor.compute_log_prob(data=data, calculate_entropy=True)
            output = DataProto.from_dict(
                tensors={"old_log_probs": output, "entropys": entropys, "old_topk_probs": topk_probs, "old_topk_indices":topk_ids,"topk_logits":topk_logits },
