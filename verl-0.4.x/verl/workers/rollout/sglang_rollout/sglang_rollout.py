...
def _set_envs_and_config(server_args: ServerArgs):
    ...
    if server_args.tp_size * server_args.dp_size > 1:
        # Keep this workaround until the Triton cache manager fix is available in the dependency.
        maybe_set_triton_cache_manager()
...
class AsyncEngine(sglang.srt.entrypoints.engine.Engine):
    """Async wrapper used by the hybrid engine to release and reload SGLang memory."""
...
def _pre_process_inputs(
    ...
    """Remove left padding from prompt token IDs before sending them to SGLang."""
    # remove the left padding in the prompt token_id using the attention mask
    non_pad_indices = torch.nonzero(attention_mask == 1, as_tuple=False)
    if non_pad_indices.numel() == 0:
        return prompt_token_ids.tolist()
    first_idx = non_pad_indices[0][0]
    token_ids = prompt_token_ids[first_idx:].tolist()
    return token_ids
...
def _post_process_outputs(tokenizer, output):
    def _map_each_response(resp):
        ...
        output_topk_original_prob_list = resp["meta_info"]["output_topk_original_prob_list"]
        output_topk_original_indice_list = resp["meta_info"]["output_topk_original_indice_list"]
        ...
        return torch.tensor(output_token_ids), torch.tensor(log_probs), torch.tensor(
            output_topk_idx_list), torch.tensor(output_topk_gumbel_list), torch.tensor(output_topk_original_prob_list), torch.tensor(output_topk_original_indice_list)
    ...
    batched_logprobs = []
    batched_output_topk_original_probs = []
    batched_output_topk_original_indices = []
    for output_token_ids, log_probs, idx, gumbel, original_prob, original_indice in out_map:
        batched_output_topk_ids.append(idx)
        batched_output_topk_gumbels.append(gumbel)
        batched_output_token_ids.append(output_token_ids)
        batched_logprobs.append(log_probs)
        batched_output_topk_original_probs.append(original_prob)
        batched_output_topk_original_indices.append(original_indice)
    ...
    if len(batched_logprobs) > 0:
        ...
        batched_output_topk_original_probs = pad_sequence(batched_output_topk_original_probs, batch_first=True, padding_value=pad_token_id)
        batched_output_topk_original_indices = pad_sequence(batched_output_topk_original_indices, batch_first=True, padding_value=pad_token_id)
    return batched_output_token_ids, batched_logprobs, batched_output_topk_ids, batched_output_topk_gumbels, batched_output_topk_original_probs, batched_output_topk_original_indices
    ...
    def _init_inference_engine(self, trust_remote_code, actor_module, port):
        ...
        if first_rank_in_node:
            ...
            self._engine = AsyncEngine(
                ...
                enable_latent=self.config.get("enable_latent", True),
                latent_end_token_id=self.config.get("latent_end_token_id", 524),
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if "raw_prompt_ids" not in non_tensor_batch:
            non_tensor_batch["raw_prompt_ids"] = np.array(
                 [_pre_process_inputs(attention_mask[i], idx[i]) for i in range(batch_size)],
                dtype=object,
            )
        ...
        with self.update_sampling_params(**kwargs):
            ...
            response = out[0].to(idx.device) #B:O
            rollout_log_probs = out[1].to(idx.device) #B:O

            rollout_topk_ids = out[2].to(idx.device)#B:O:K
            rollout_topk_gumbels = out[3].to(idx.device)#B:O:K
            rollout_topk_original_probs = out[4].to(idx.device)#B:O:K
            rollout_topk_original_indices = out[5].to(idx.device)#B:O:K
            K = rollout_topk_ids.size(-1)
            idx_topk = torch.full_like(idx.unsqueeze(-1).repeat(1, 1, K), fill_value=-100, device=idx.device)#B:I:K
            idx_topk[..., 0] = idx
            if response.shape[1] < self.config.response_length:
                ...
                rollout_topk_gumbels = pad_sequence_to_length_1(rollout_topk_gumbels, self.config.response_length,
                                                                0.0)
                rollout_topk_original_probs = pad_sequence_to_length_1(rollout_topk_original_probs, self.config.response_length,
                                                                0.0)
                rollout_topk_original_indices = pad_sequence_to_length_1(rollout_topk_original_indices, self.config.response_length,
                                                                -100)

            # utilize current sampling params
            if self.sampling_params.get("n", 1) > 1 and do_sample:
                idx = idx.repeat_interleave(self.sampling_params["n"], dim=0)#B*n:I
                idx_topk = idx_topk.repeat_interleave(self.sampling_params["n"], dim=0)#B*n:I:K
            ...
            rollout_topk_ids = torch.cat([idx_topk, rollout_topk_ids], dim=1)#B*n:I+O:K
            rollout_topk_gumbels = torch.cat(
                [torch.zeros_like(idx_topk, dtype=rollout_topk_gumbels.dtype), rollout_topk_gumbels], dim=1)
            rollout_topk_original_probs = torch.cat(
                [torch.zeros_like(idx_topk, dtype=rollout_topk_original_probs.dtype), rollout_topk_original_probs], dim=1)
            rollout_topk_original_indices = torch.cat(
                [torch.zeros_like(idx_topk, dtype=rollout_topk_original_indices.dtype), rollout_topk_original_indices], dim=1)
        ...
        batch = TensorDict(
            {
                ...
                "rollout_topk_original_probs": rollout_topk_original_probs,  # we will recompute old log prob with actor
                "rollout_topk_original_indices": rollout_topk_original_indices,  # we will recompute old log prob with actor
                "attention_mask": attention_mask,
                "position_ids": position_ids,
                "gumbel_temperature": torch.full(
                    (batch_size,),
                    float(self.config.get("gumbel_softmax_temperature", 1.0)),
        ...
        # Propagate sampling params to meta_info for PPO recomputation consistency
        # Use the actual temperature/top_p used in this generation
        meta_info = {
        ...
        return DataProto(batch=batch, non_tensor_batch=_non_tensor_batch, meta_info=meta_info)
        ...
        for data_idx, raw_prompt in enumerate(prompts.non_tensor_batch["raw_prompt"]):
            for rollout_offset in range(n):
                if self._tool_schemas:
                    ...
                    prompt_attention_mask = prompts.batch["attention_mask"][data_idx]
                    _input_ids = _pre_process_inputs(prompt_attention_mask, prompts.batch["input_ids"][data_idx])
                    _attention_mask = _pre_process_inputs(prompt_attention_mask, prompt_attention_mask)
